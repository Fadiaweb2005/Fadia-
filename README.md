...
// ตัวอย่างข้อมูลจำลองที่ถอดโครงสร้างมาจาก CSV ของคุณ
// ในการใช้งานจริง ข้อมูลส่วนนี้จะถูก parse จากไฟล์สรุปหนังสือ - Sheet1.csv
const RAW_DATA = [
  { grade: "ป.1", class: "1/1", subject: "คณิต", total: 30, received: 30, status: "ครบ" },
  { grade: "ป.1", class: "1/2", subject: "คณิต", total: 26, received: 24, status: "ขาด 2 เล่ม" },
  { grade: "ป.1", class: "1/1", subject: "ภาษาไทย", total: 30, received: 30, status: "ครบ" },
  { grade: "ป.1", class: "1/2", subject: "ภาษาไทย", total: 26, received: 25, status: "ขาด 1 เล่ม" },
  { grade: "ป.2", class: "2/1", subject: "คณิต", total: 30, received: 30, status: "ครบ" },
  { grade: "ป.2", class: "2/2", subject: "คณิต", total: 30, received: 30, status: "ครบ" },
  { grade: "ป.2", class: "2/1", subject: "วิทยาการคำนวณ", total: 30, received: 30, status: "ครบ" },
  { grade: "ป.2", class: "2/2", subject: "วิทยาการคำนวณ", total: 30, received: 26, status: "ขาด 4 เล่ม" },
  { grade: "ป.3", class: "3/1", subject: "วิทยาศาสตร์", total: 27, received: 27, status: "ครบ" },
  { grade: "ป.3", class: "3/2", subject: "วิทยาศาสตร์", total: 24, received: 22, status: "ขาด 2 เล่ม" },
  { grade: "ป.4", class: "4/1", subject: "สังคม", total: 26, received: 26, status: "ครบ" },
  { grade: "ป.4", class: "4/2", subject: "สังคม", total: 28, received: 1, status: "ขาด 27 เล่ม" },
];
...
const App = () => {
  ...
  // สกัดรายชื่อวิชาและชั้นเรียนที่ไม่ซ้ำกันเพื่อทำ Filter
  const subjects = useMemo(() => ['All', ...new Set(RAW_DATA.map(item => item.subject))], []);
  const grades = useMemo(() => ['All', ...new Set(RAW_DATA.map(item => item.grade))], []);
  ...
  const filteredData = useMemo(() => {
    return RAW_DATA.filter(item => {
      ...
      const matchSearch = item.subject.includes(searchTerm) || 
                          `ป.${item.class}`.includes(searchTerm);
      return matchSubject && matchGrade && matchSearch;
    });
  }, [subjectFilter, gradeFilter, searchTerm]);

  // คำนวณสรุปผล
  const stats = useMemo(() => {
    const total = filteredData.reduce((acc, curr) => acc + curr.total, 0);
    const received = filteredData.reduce((acc, curr) => acc + curr.received, 0);
    const missing = total - received;
    const completeCount = filteredData.filter(d => d.total === d.received).length;
    
    return { total, received, missing, completeCount, totalEntries: filteredData.length };
  }, [filteredData]);
